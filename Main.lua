...
------------------------------------------------
-- KEY SPEICHERN / LADEN (24 Stunden)
------------------------------------------------
...
local function loadSavedKey()
    local ok, result = pcall(function()
        ...
        -- Prüfen ob weniger als 24 Stunden vergangen sind
        local elapsed = os.time() - data.savedAt
        if elapsed > 86400 then -- 86400 Sekunden = 24 Stunden
...
local function clearSavedKey()
    pcall(function()
        if isfile(SAVE_FILE) then deletefile(SAVE_FILE) end
    end)
end
...
Players.PlayerAdded:Connect(function(plr)
    plr.CharacterAdded:Connect(function(char)
        char:WaitForChild("Humanoid").Died:Connect(function()
            if _G.KillOn and plr~=LocalPlayer then
                playSnd(_G.KillId, _G.KillVol, 1)
            end
        end)
    end)
end)
...
------------------------------------------------
-- SPEED SYSTEM (KOMPLETT NEU GEBAUT)
-- Problem war: Roblox/TSB setzt WalkSpeed über
-- Animationen und Humanoid-States zurück.
-- Lösung: Wir setzen Speed über RunService
-- kontinuierlich und auch nach State-Änderungen
------------------------------------------------
...
local function applySpeedNow()
    ...
    if target ~= 16 or _G.SpeedOn then
...
local function setupSpeedLoop(active)
    ...
    -- Jede Sekunde Speed setzen (verhindert Reset)
    speedConnection = RunService.Heartbeat:Connect(function()
        ...
        if not hum then return end
        local target = _G.SpeedOn and 28 or _G.SpeedVal
        if hum.WalkSpeed ~= target and hum.Health > 0 then
            hum.WalkSpeed = target
        end
    end)
end

-- Speed nach Respawn wiederherstellen
LocalPlayer.CharacterAdded:Connect(function(char)
    ...
    if hum then
        ...
        -- StateChange: Manche Spiele setzen Speed bei Stateänderung zurück
        stateConnection = hum.StateChanged:Connect(function(old, new)
            task.wait(0.05) -- kurz warten dann überschreiben
            local t = _G.SpeedOn and 28 or _G.SpeedVal
            if hum and hum.Parent and t ~= 16 then
                hum.WalkSpeed = t
            end
        end)
    end
    -- Auch beim neuen Char Speed Loop starten wenn aktiv
    if _G.SpeedOn or _G.SpeedVal ~= 16 then
        setupSpeedLoop(true)
    end
end)
...
------------------------------------------------
-- EMOTES (NEU - Zuverlässige Version)
-- Roblox Standard Emotes werden über den
-- Emote-Command gespielt, nicht manuell geladen
------------------------------------------------
local EMOTES = {
    {sym="♛", name="Verbeugung",  cmd="wave"},
    {sym="◆", name="Jubeln",      cmd="cheer"},
    {sym="≋", name="Tanzen 1",    cmd="dance"},
    {sym="⊕", name="Tanzen 2",    cmd="dance2"},
    {sym="✦", name="Tanzen 3",    cmd="dance3"},
    {sym="⟳", name="Lachen",      cmd="laugh"},
    {sym="➤", name="Zeigen",      cmd="point"},
}

local curEmoteTrack = nil

local function playEmote(cmd)
    local char = LocalPlayer.Character
    if not char then return end
    local hum = char:FindFirstChildOfClass("Humanoid")
    ...
    -- Vorherige Emote stoppen
    if curEmoteTrack then
        pcall(function() curEmoteTrack:Stop(0.2) end)
        curEmoteTrack = nil
    end

    -- Emote IDs (diese funktionieren zuverlässig in Roblox)
    local emoteIds = {
    ...
    task.spawn(function()
        ...
        if not ok then
            -- Fallback: Roblox eingebauter Emote Chat Command
            pcall(function()
...
local GFX = {
    POTATO=function()
        clearFX() settings().Rendering.QualityLevel=Enum.QualityLevel.Level01
    ...
    LOW=function()
        clearFX() settings().Rendering.QualityLevel=Enum.QualityLevel.Level03 Lighting.GlobalShadows=false
    end,
    MEDIUM=function()
...
GUI.Name="GodValleyV7" GUI.ResetOnSpawn=false
...
OBImg.BackgroundTransparency=1 OBImg.Image=IMG.luffy OBImg.ScaleType=Enum.ScaleType.Fit OBImg.ZIndex=11
...
local function buildUI(keyType)
    ...
    do
        ...
        if keyType=="admin" then
            ...
            local hs=Instance.new("TextLabel",Hdr) hs.Size=UDim2.new(0,210,0,13) hs.Position=UDim2.new(0,59,0,36) hs.BackgroundTransparency=1 hs.Text="TSB "..S.wave.." v7.0 "..S.wave.." One Piece" hs.TextColor3=C.sub hs.Font=Enum.Font.Gotham hs.TextSize=FS hs.TextXAlignment=Enum.TextXAlignment.Left hs.ZIndex=22
        end
        -- Key löschen Button (für Logout)
        local logoutBtn=Instance.new("TextButton",Hdr)
        logoutBtn.Size=UDim2.new(0,44,0,16) logoutBtn.Position=UDim2.new(1,-120,0.5,-8)
        logoutBtn.BackgroundColor3=Color3.fromRGB(30,12,12) logoutBtn.Text=S.ban.." Logout"
        logoutBtn.TextColor3=C.sub logoutBtn.Font=Enum.Font.Gotham logoutBtn.TextSize=9 logoutBtn.ZIndex=23
        Instance.new("UICorner",logoutBtn).CornerRadius=UDim.new(0,5)
    ...
    tog("Battle",S.bolt,"Speed Boost","WalkSpeed auf 28",function(on)
        ...
        if on then
            ...
            _G.SpeedVal = 16
            setupSpeedLoop(false)
            applySpeedNow()
        end
    end)

    sld("Battle",S.arrow,"Custom Speed",16,100,16,function(val)
        _G.SpeedVal = val
        _G.SpeedOn = (val > 16)
        applySpeedNow()
        setupSpeedLoop(_G.SpeedOn)
    end)

    tog("Battle",S.eye,"Max Zoom","Kamera rauszoomen",function(on) LocalPlayer.CameraMaxZoomDistance=on and 200 or 30 end)
    tog("Battle",S.world,"No Clip","Durch Wände gehen",function(on)
    ...
    tog("Battle",S.music,"Kill Sound","Sound bei Kill",function(on) _G.KillOn=on _G.KillId=4612365796 end)
    ...
    tog("Battle",S.shield,"Low HP Alarm","Alarm bei wenig HP",function(on)
        task.spawn(function()
    ...
    do
        ...
        for _,em in ipairs(EMOTES) do
            ...
            b.MouseEnter:Connect(function()
                TweenService:Create(b,TweenInfo.new(0.14),{BackgroundColor3=C.cardH}):Play()
                TweenService:Create(bsk,TweenInfo.new(0.14),{Transparency=0.55}):Play()
            end)
            b.MouseLeave:Connect(function()
                TweenService:Create(b,TweenInfo.new(0.14),{BackgroundColor3=C.card}):Play()
                TweenService:Create(bsk,TweenInfo.new(0.14),{Transparency=1}):Play()
            end)

            local ecmd = em.cmd
            b.MouseButton1Click:Connect(function()
                playEmote(ecmd)
                -- Bounce + Farb Animation
...
------------------------------------------------
-- KEY UNLOCK FUNKTION (wird von beiden Pfaden benutzt)
------------------------------------------------
local function unlockHub(kType, savedKey)
    buildUI(kType)
    ...
    local tT=Instance.new("TextLabel",toast) tT.Size=UDim2.new(1,-52,0,18) tT.Position=UDim2.new(0,50,0,7) tT.BackgroundTransparency=1 tT.Text=S.anchor.." GOD VALLEY HUB v7.0" tT.TextColor3=C.acc tT.Font=Enum.Font.GothamBold tT.TextSize=isMobile and 11 or 12 tT.TextXAlignment=Enum.TextXAlignment.Left tT.ZIndex=201
    local tS2=Instance.new("TextLabel",toast) tS2.Size=UDim2.new(1,-52,0,13) tS2.Position=UDim2.new(0,50,0,27) tS2.BackgroundTransparency=1
    tS2.Text = savedKey and (S.check.." Willkommen zurück!") or (kType=="admin" and (S.crown.." Admin Zugang!") or (S.check.." Zugang gewährt!"))
...
if savedKey then
    ...
    if kType then
        -- Key noch gültig — direkt überspringen
        KS.Visible = false
        task.wait(0.5)
...
if KS.Visible then
    ...
    local KTit=Instance.new("TextLabel",KS) KTit.Size=UDim2.new(0.8,0,0,40) KTit.Position=UDim2.new(0.1,0,0,isMobile and 55 or 70) KTit.BackgroundTransparency=1 KTit.Text=S.anchor.."  GOD VALLEY HUB  "..S.anchor KTit.Font=Enum.Font.GothamBold KTit.TextSize=isMobile and 20 or 26 KTit.ZIndex=52
    ...
    local KSub=Instance.new("TextLabel",KS) KSub.Size=UDim2.new(0.8,0,0,18) KSub.Position=UDim2.new(0.1,0,0,isMobile and 100 or 118) KSub.BackgroundTransparency=1 KSub.Text=S.wave.." The Strongest Battleground "..S.wave.." One Piece Edition" KSub.TextColor3=C.sub KSub.Font=Enum.Font.Gotham KSub.TextSize=isMobile and 10 or 12 KSub.ZIndex=52

    local KW=isMobile and 280 or 355
    local KH=isMobile and 255 or 285
    ...
    local WBtn=Instance.new("TextButton",KB) WBtn.Size=UDim2.new(1,-20,0,isMobile and 34 or 38) WBtn.Position=UDim2.new(0,10,0,70) WBtn.BackgroundColor3=Color3.fromRGB(18,10,4) WBtn.TextColor3=C.acc WBtn.Font=Enum.Font.GothamBold WBtn.TextSize=isMobile and 10 or 11 WBtn.Text=S.anchor.."  Website öffnen  "..S.arrow.."  Key holen" WBtn.ZIndex=53
    ...
    task.spawn(function() while KS.Visible do TweenService:Create(WBS,TweenInfo.new(1,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Transparency=0.7}):Play() task.wait(1) TweenService:Create(WBS,TweenInfo.new(1,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{Transparency=0}):Play() task.wait(1) end end)

    WBtn.MouseButton1Click:Connect(function()
        uiSnd(1.3)
        pcall(function() setclipboard(KEY_WEBSITE) end)
        local orig=WBtn.Text WBtn.Text=S.check.."  Link kopiert! Im Browser öffnen" WBtn.TextColor3=C.green
        task.wait(2.5) WBtn.Text=orig WBtn.TextColor3=C.acc
    end)

    local StepsLbl=Instance.new("TextLabel",KB) StepsLbl.Size=UDim2.new(1,-20,0,36) StepsLbl.Position=UDim2.new(0,10,0,114) StepsLbl.BackgroundTransparency=1 StepsLbl.Text=S.diamond.." 1. Website öffnen\n"..S.arrow.." 2. Aufgaben abschließen\n"..S.check.." 3. Key kopieren & hier eingeben" StepsLbl.TextColor3=C.sub StepsLbl.Font=Enum.Font.Gotham StepsLbl.TextSize=isMobile and 8 or 9 StepsLbl.TextWrapped=true StepsLbl.TextXAlignment=Enum.TextXAlignment.Left StepsLbl.ZIndex=53

    local InBg=Instance.new("Frame",KB) InBg.Size=UDim2.new(1,-20,0,isMobile and 32 or 36) InBg.Position=UDim2.new(0,10,0,156) InBg.BackgroundColor3=C.card InBg.BorderSizePixel=0 InBg.ZIndex=53
    ...
    local KIn=Instance.new("TextBox",InBg) KIn.Size=UDim2.new(1,-16,1,0) KIn.Position=UDim2.new(0,8,0,0) KIn.BackgroundTransparency=1 KIn.PlaceholderText=S.lock.." FREE_... oder ADMIN Key" KIn.PlaceholderColor3=C.sub KIn.Text="" KIn.TextColor3=C.text KIn.Font=Enum.Font.Gotham KIn.TextSize=isMobile and 9 or 11 KIn.ClearTextOnFocus=false KIn.ZIndex=54
    ...
    local StLbl=Instance.new("TextLabel",KB) StLbl.Size=UDim2.new(1,-20,0,13) StLbl.Position=UDim2.new(0,10,0,196) StLbl.BackgroundTransparency=1 StLbl.Text="" StLbl.TextColor3=C.red StLbl.Font=Enum.Font.Gotham StLbl.TextSize=FS StLbl.ZIndex=53

    local EnBtn=Instance.new("TextButton",KB) EnBtn.Size=UDim2.new(1,-20,0,isMobile and 30 or 34) EnBtn.Position=UDim2.new(0,10,0,212) EnBtn.BackgroundColor3=C.acc2 EnBtn.TextColor3=C.white EnBtn.Font=Enum.Font.GothamBold EnBtn.TextSize=isMobile and 11 or 13 EnBtn.Text=S.arrow.."  BESTÄTIGEN" EnBtn.ZIndex=53
    Instance.new("UICorner",EnBtn).CornerRadius=UDim.new(0,9)
    task.spawn(function() while KS.Visible do TweenService:Create(EnBtn,TweenInfo.new(0.9,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{BackgroundColor3=Color3.fromRGB(200,55,30)}):Play() task.wait(0.9) TweenService:Create(EnBtn,TweenInfo.new(0.9,Enum.EasingStyle.Sine,Enum.EasingDirection.InOut),{BackgroundColor3=C.acc2}):Play() task.wait(0.9) end end)

    ------------------------------------------------
    -- KEY VALIDIERUNG + SPEICHERN
    ------------------------------------------------
    local function tryKey(raw)
        ...
        if kType then
            ...
            TweenService:Create(LB,TweenInfo.new(0.3,Enum.EasingStyle.Back),{Size=UDim2.new(0,52,0,52),Position=UDim2.new(0.5,-26,0,-28)}):Play()
            ...
            TweenService:Create(KS,TweenInfo.new(0.5,Enum.EasingStyle.Quad,Enum.EasingDirection.In),{Position=UDim2.new(0,0,-1,0)}):Play()
            task.wait(0.55)
            ...
            StLbl.Text=S.cross.." Ungültig! Website besuchen."
